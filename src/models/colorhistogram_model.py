...
class ColorHistogram_Model(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.input_A_Seg, self.input_B_Seg, self.input_SegNum = self.MakeLabelFromMap(
            self.input_A_Map, self.input_B_Map
        )
        ...
        with torch.no_grad():
            ...
            if (self.inp.size(2) > 700) or (self.inp.size(3) > 700):
                ...
                if self.inp.size(2) > self.inp.size(3):
                    self.inp = F.upsample(
                        self.inp, size=(700, int(700 / aspect_ratio)), mode="bilinear"
                    )
                else:
                    self.inp = F.upsample(
                        self.inp, size=(int(700 * aspect_ratio), 700), mode="bilinear"
                    )
            # max 700 px with aspect ratio
            if (self.tar.size(2) > 700) or (self.tar.size(3) > 700):
                ...
                if self.tar.size(2) > self.tar.size(3):
                    self.tar = F.upsample(
                        self.tar, size=(700, int(700 / aspect_ratio)), mode="bilinear"
                    )
                else:
                    self.tar = F.upsample(
                        self.tar, size=(int(700 * aspect_ratio), 700), mode="bilinear"
                    )

            # In case of mis-alignment
            self.A_seg = (
                F.upsample(
                    self.A_seg.unsqueeze(0).float(),
                    size=(self.inp.size(2), self.inp.size(3)),
                    mode="bilinear",
                )
                .squeeze(0)
                .long()
            ...
            self.B_seg = (
                F.upsample(
                    self.B_seg.unsqueeze(0).float(),
                    size=(self.tar.size(2), self.tar.size(3)),
                    mode="bilinear",
                )
                .squeeze(0)
                .long()
            ...
            with torch.no_grad():
                ...
                hist_inp_l = self.getHistogram1d_np(self.inp, self.hist_l).repeat(
                    1, 1, self.hist_ab, self.hist_ab
                )
                hist_inp = torch.cat((hist_inp_ab, hist_inp_l), 1)

                hist_tar_ab = self.getHistogram2d_np(self.tar, self.hist_ab)
                hist_tar_l = self.getHistogram1d_np(self.tar, self.hist_l).repeat(
                    1, 1, self.hist_ab, self.hist_ab
                )
                ...
                hist_inp_feat_tile = hist_inp_feat.repeat(
                    1, 1, self.inp.size(2), self.inp.size(3)
                )
                hist_tar_feat_tile = hist_tar_feat.repeat(
                    1, 1, self.tar.size(2), self.tar.size(3)
                )
                self.final_result_tar = hist_tar_feat.repeat(
                    1, 1, self.inp.size(2), self.inp.size(3)
                )
            ...
            if self.opt.is_SR:
                for i in range(0, self.input_SegNum):
                    ...
                    if (seg_num_A > 0) and (seg_num_B > 0):
                        self.segmentwise_tile(
                            self.tar, self.B_seg, self.A_seg, self.final_result_tar, i
                        )
            ...
            self.inp = self.inp[
                :,
                :,
                self.pad : (self.inp.size(2) - 2 * self.pad),
                self.pad : (self.inp.size(3) - 2 * self.pad),
            ]
            self.tar = self.tar[
                :,
                :,
                self.pad : (self.tar.size(2) - 2 * self.pad),
                self.pad : (self.tar.size(3) - 2 * self.pad),
            ]
            self.out = out[
                :,
                :,
                self.pad : (out.size(2) - 2 * self.pad),
                self.pad : (out.size(3) - 2 * self.pad),
            ]

    def get_current_visuals(self):
        ret_visuals = OrderedDict(
            [
                ("01_input", util.tensor2im(self.inp, self.img_type)),
    ...
    def getHistogram2d_np(self, img_torch, num_bin):
        ...
        # Normalize
        total_num = sum(
            sum(H_torch.squeeze(0).squeeze(0))
        )  # 256 * 256 => same value as arr[0][0].ravel()[np.flatnonzero(arr[0][0])].shape
        H_torch = H_torch / total_num

        return H_torch

    def getHistogram1d_np(
        self, img_torch, num_bin
    ):  # L space # Idon't know why but they(np, conv) are not exactly same
        # Preprocess
        arr = img_torch.detach().cpu().numpy()
    ...
    def segmentwise_tile(self, img, seg_src, seg_tgt, final_tensor, segment_num):
        ...
        with torch.no_grad():
            ...
            hist_1d = self.getHistogram1d_np(mask_seg, self.hist_l).repeat(
                1, 1, self.hist_ab, self.hist_ab
            )
        ...
        final_tensor[:, :, seg_tgt.squeeze(0) == segment_num] = (
            hist_feat.repeat(
                1, 1, final_tensor[:, :, seg_tgt.squeeze(0) == segment_num].size(2), 1
            )
            .squeeze(0)
            .permute(2, 0, 1)
        )
    ...
    def MakeLabelFromMap(self, input_A_Map, input_B_Map):
        ...
        for i in range(0, label_AB.size(0)):
            A_seg[
                (
                    input_A_Map.squeeze(0)
                    == label_AB[i].unsqueeze(0).unsqueeze(0).permute(2, 0, 1)
                )[0:1, :, :]
            ] = i
            B_seg[
                (
                    input_B_Map.squeeze(0)
                    == label_AB[i].unsqueeze(0).unsqueeze(0).permute(2, 0, 1)
                )[0:1, :, :]
            ] = i
