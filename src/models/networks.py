...
class ConditionNetwork2(nn.Module):
    ...
    def __init__(
        ...
        ngf : int, optional
            Number of gen filters in first conv layer, by default 64
        norm_layer : nn.Module, optional
            Type of normalization layer, by default nn.BatchNorm2d
        use_dropout : Boolean, optional
            If use dropout, by default False # TODO useless?
        gpu_ids : List, optional
            List of GPU ids, by default []
        padding_type : str, optional
            Padding type, by default "reflect" # TODO useless?
        """
        ...
        model = [
            #### Half Size
            nn.Conv2d(
                self.input_nc, dim2, kernel_size=4, padding=1, stride=2, bias=use_bias
            ),
...
class HISTUnet3_Res(nn.Module):
    ...
    def __init__(
        ...
        output_nc : int
            Number of output channels
        ngf : int, optional
            Number of gen filters in first conv layer, by default 64 # TODO useless?
        norm_layer : nn.Module, optional
            Type of normalization layer, by default nn.BatchNorm2d
        use_dropout : Boolean, optional
            If use dropout, by default False
        padding_type : int, optional
            Padding type, by default "zero"
        """
        ...
        # Init
        self.block_init = self.build_init_block(
            input_nc, enc_nc, padding_type, use_bias
        )
        ...
        # Dec
        self.HistUnetDec1 = UnetDec_deep2(
            512 + 512 + enc_nc + enc_nc, 512, norm_layer
        )  # 512 + 512 + 72 + 72
        ...
        # Res
        self.ENC_Block1 = ENCResnetBlock(
            enc_nc, padding_type, norm_layer, use_dropout, use_bias
        )
        self.ENC_Block2 = ENCResnetBlock(
            enc_nc, padding_type, norm_layer, use_dropout, use_bias
        )
        self.ENC_Block3 = ENCResnetBlock(
            enc_nc, padding_type, norm_layer, use_dropout, use_bias
        )
    ...
    def build_init_block(
        self, input_nc: int, dim_img: int, padding_type: str, use_bias: Boolean
    ) -> nn.Module:  # 3 -> 64
        """Build initial block
        ...
        input_nc : int
            Number of input channels
        dim_img : int
            Taget image dimension
        padding_type : str
            Padding type
        use_bias : Boolean
            If use bias # TODO useless?
        ...
        ------
        NotImplementedError
            Specified padding type is not implemented.
        """
        block_init = []
        ...
        block_init += [
            nn.Conv2d(input_nc, dim_img, kernel_size=3, padding=p, stride=1),
    ...
    def build_inter_out(self, dim_img, dim_out, padding_type, use_bias):
        ...
        block_last += [
            nn.Conv2d(dim_img, dim_out, kernel_size=3, padding=p, bias=use_bias),
        ]
        return nn.Sequential(*block_last)
    ...
    def forward(self, input_img, hist1_enc, hist2_enc):
        ...
        # Enc
        mid_img1, mid_img2, mid_img3, mid_img4, mid_img5 = self.HistUnetEnc(
            input_img
        )  # 1/3/256/256

        # Dec
        out_img1 = self.HistUnetDec1(
            mid_img5, mid_img5, hist1_enc, hist2_enc, mid_img5.size(2), mid_img5.size(3)
        )  #
        out_img2 = self.HistUnetDec2(
            out_img1, mid_img4, hist1_enc, hist2_enc, mid_img4.size(2), mid_img4.size(3)
        )  #
        out_img3 = self.HistUnetDec3(
            out_img2, mid_img3, hist1_enc, hist2_enc, mid_img3.size(2), mid_img3.size(3)
        )  #
        out_img4 = self.HistUnetDec4(
            out_img3, mid_img2, hist1_enc, hist2_enc, mid_img2.size(2), mid_img2.size(3)
        )  #
        out_img5 = self.HistUnetDec5(
            out_img4, mid_img1, hist1_enc, hist2_enc, mid_img1.size(2), mid_img1.size(3)
        )  #
        out_img5 = F.upsample(
            out_img5, size=(input_img.size(2), input_img.size(3)), mode="bilinear"
        )

        out_img6 = self.ENC_Block1.forward(
            out_img5 + self.block_init(input_img), hist1_enc, hist2_enc
        )
        out_img7 = self.ENC_Block2.forward(
            out_img6 + self.block_init(input_img), hist1_enc, hist2_enc
        )
        out_img = self.ENC_Block3.forward(
            out_img7 + self.block_init(input_img), hist1_enc, hist2_enc
        )
...
class UnetEnc_deep2(nn.Module):
    ...
    # TODO Can be merged with block1
...
class UnetDec_deep1(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, use_tanh, norm_layer=nn.BatchNorm2d):
        ...
        self.block = self.build_block(
            input_nc, output_nc, use_tanh, norm_layer=norm_layer
        )
...
class UnetDec_deep2(nn.Module):
    ...
    def build_block(self, input_nc, output_nc, norm_layer):
        ...
        block_full = [
            ...
            nn.Upsample(scale_factor=2, mode="bilinear"),
            nn.Conv2d(
                input_nc, output_nc, kernel_size=3, stride=1, padding=1, bias=use_bias
            ),
            nn.InstanceNorm2d(output_nc),
            # 2
            nn.ReLU(True),
...
class ENCResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [
            nn.Conv2d(dim * 3, dim * 3, kernel_size=3, padding=p, bias=use_bias),
            norm_layer(dim * 3),
            nn.ReLU(True),
        ]
        if use_dropout:
        ...
        if padding_type == "reflect":
            ...
            raise NotImplementedError("padding [%s] is not implemented" % padding_type)
        conv_block += [
            nn.Conv2d(dim * 3, dim, kernel_size=3, padding=p, bias=use_bias),
            norm_layer(dim),
        ]
